...
Batch 5 (design doc v2: WS1–WS4) is implemented, migrated (00005 backfill + 00009–00014 applied to the live project), demo-seeded, and deployed via push to `main` (Vercel auto-deploy). The app is in **demo/test form** — demo logins all use password `Finesse4Blinds!`: `demo.sales@finessett.com` (salesman), `demo.retail@finessett.com` (retail), `demo.wholesale@finessett.com` (wholesale). Re-run the demo dataset any time with `npx tsx scripts/seed-demo.ts` (idempotent; recreates the two demo properties and their quotes/jobs).

Next session should: (a) collect client feedback from the demo walkthrough, (b) consider real product photography for the gallery (admin can upload via Admin → Blind/Awning Products), (c) confirm the retail 40% / wholesale 20% markup defaults and §6 formulas against `FinessePortal - Formulas.xlsx` when it syncs, and (d) decide on an email provider for quote-send (link-only ships today).

Waiting on the user for: demo review with the client; markup/formula confirmation; token rotation (two GitHub PATs + one Supabase PAT from earlier sessions, still unrotated).
